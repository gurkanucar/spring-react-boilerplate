...
public class ProcessScheduledEventUseCase {
    ...
    public void execute(UUID id) {
        ScheduledEvent event = repository.findById(id).orElse(null);
        if (event == null) {
            log.warn("[SCHEDULED-EVENT] delivered id={} has no row (cancelled + purged?) — dropping", id);
            return;
        }
        if (event.getStatus() == ScheduledEventStatus.CANCELLED) {
            log.info("[SCHEDULED-EVENT] id={} was cancelled — skipping", id);
            return;
        }
        if (event.getStatus() == ScheduledEventStatus.DELIVERED) {
